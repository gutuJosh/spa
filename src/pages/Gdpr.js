         ...
         <div className="wrapper">
          <div className="flex-grid">
           <div className="flex-lg-12 flex-md-12 flex-sm-12">
             <h2 className="text-white">{props.t('gdpr:Conformità GDPR di Bancomail')}</h2>
             <p className="text-white">{props.t('gdpr:In Bancomail, il rispetto delle normative è fondamentale, per questo da sempre ci impegniamo a conoscere e rispettare la normativa vigente ed applicabile per la tutela dei contatti nel nostro Database')}</p>
           </div>
          </div>
         ...
         <div className="page-body flex-grid">
          <div className="flex-lg-12 flex-md-12 flex-sm-12">
            ...
            <List type="skyblue" number="2"> 
              ...
              <p>
              {props.t('gdpr:È un caso molto meno frequente nel nostro Database, ma esistente.')} {props.t('gdpr:Dati di contatto che incidentalmente identificano la “persona fisica” possono presentarsi secondo due pattern')}: 
              <br/>
              ...
              <p>
                {props.t('gdpr:Mentre nel primo caso, la persona è pur chiaramente identificata dall’attività aziendale cui è relazionata; il secondo caso raccoglie per lo più Liberi Professionisti in forma singola che utilizzano, pur a scopo Business, dati di contatto che identificano chiaramente la “persona fisica” anche al di là della sua veste professionale.')}&nbsp;
              ...
              <ul>
                <li>
                    ...
                    <p>{props.t('gdpr:Se l’indirizzo email (o il nome e cognome, la carica, etc.) sono stati conferiti dal soggetto, indicandoli come dati di contatto per la propria attività, la stessa azione li fa chiaramente rientrare nell’esclusione del Considerando 14.')}</p>
                    </Thumb>
                </li>
                <li>
                 <Thumb color="text-grey" number="B">
                     <p>{props.t('gdpr:Se invece (aldilà delle intenzioni del conferente) si vogliono considerare questi dati come riferiti alla “Natural Person”, il diritto al trattamento ci viene assicurato da altri due Considerando della Legge')}: {props.t('gdpr:il 47 e il 70.')}</p>
                 </Thumb>  
                </li>
              </ul>
              ...
              <p>{props.t('gdpr:Quindi, se il legittimo interesse è finalizzato al direct marketing, il trattamento di questo tipo di dati è consentito.')} {props.t('gdpr:In questo caso però l’assicurazione dei diritti all’interessato non è più una scelta, ma un obbligo.')} {props.t('gdpr:Questo significa informare chiaramente il destinatario del trattamento e delle sue finalità assicurandogli l’esercizio dei diritti.')}</p>
              <p>{props.t('gdpr:La trasmissione dell’informativa, è l’occasione stessa in cui presentare la propria attività e l\'oggetto della eventuale promozione.')}</p>
            </List>
            <h3 className="mtop20">{props.t('gdpr:Altre cose che facciamo per assicurare la compatibilità con il GDPR')}:</h3>
            <List type="skyblue" number="1"> 
             <p><strong>DPO:</strong> {props.t('gdpr:nonostante il Regolamento non ci obblighi in questo senso, ci stiamo dotando di un Data Protection Officer per la supervisione di tutte le procedure relative al trattamento dei dati e il rilievo di eventuali criticità.')}</p>
            </List>
            <List type="skyblue" number="2"> 
             <p><strong>Lia:</strong> {props.t('gdpr:è un audit interno (“Legitimate Interest Assessment”) che serve a mettere su carta ed archiviare la valutazione fatta dal Team Databale e DPO, in merito al nostro legittimo interesse e al legittimo interesse dei nostri clienti.')}</p>
             <p>{props.t('gdpr:Questo documento tiene conto di un “balance test” oggettivo, per la verifica che gli interessi individuali non prevarichino il nostro.')}</p>
            </List>
            <List type="skyblue" number="3"> 
             ...
             <p>{props.t('gdpr:Questo approccio è molto importante al fine di una piena compatibilità con il Regolamento.')}</p>
            </List>
            <List type="skyblue" number="4"> 
             <p>
              <strong>ISO 27001:</strong> {props.t('gdpr:anche se non strettamente correlato al Regolamento, la nostra società si sta dotando della Certificazione ISO 27001 per la gestione della sicurezza delle informazioni.')}&nbsp;
              {props.t('gdpr:Sempre in quest’ottica i nostri sistemi sono all’avanguardia da anni, applicando tecnologie avanzate di protezione dei dati.')}
            </p>
            </List>
            <h3 className="mtop20">{props.t('gdpr:Quindi?')}</h3>
            <p>
            {props.t('gdpr:L’analisi mostra che le disposizioni del GDPR non solo non costituiscono un problema per i nostri clienti e per i marketer in generale, ma - sotto molti punti di vista - chiariscono ed ampliano le possibilità relative al direct marketing aprendo strade (es. possibilità di cessione dei dati dei “contatti” interni) che prima, per carenze normative, rimanevano nella zona d’ombra.')} 
            </p>
          </div>
